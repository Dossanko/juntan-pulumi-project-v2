...
topic_id = os.environ.get('GOLD_AND_OIL_PRICES_TOPIC_ID')

def get_price_data(symbol):
    """
    Alpha Vantage API から価格データを取得し、Pub/Sub に送信する関数
    """
    ...
    try:
        ...
        print(f"{symbol} の価格データを Pub/Sub に送信しました。")
...
def main():
    # 1時間ごとにデータを取得
    while True:
        for symbol in ["GC=F", "CL=F"]:  # 金と原油のシンボル
