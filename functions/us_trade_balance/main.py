...
topic_id = os.environ.get('US_TRADE_BALANCE_TOPIC_ID')

def get_us_trade_balance_data():
    """
    FRED API から米国貿易収支データを取得し、Pub/Sub に送信する関数
    """
    # FRED API のエンドポイント URL を設定します
    url = f"https://api.stlouisfed.org/fred/series/observations?series_id=BOPGSTB&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print("米国貿易収支データを Pub/Sub に送信しました。")
...
def main(request):
    # 最初の実行時にデータを取得
    get_us_trade_balance_data()

    # Cloud Scheduler にジョブを作成
    job_id = "get_us_trade_balance_job"
    schedule = "0 0 10 * *"  # 毎月10日の0時0分に実行
