...
topic_id = os.environ.get('JAPAN_TRADE_BALANCE_TOPIC_ID')

def get_japan_trade_balance_data():
    """
    FRED API から日本の貿易収支データを取得し、Pub/Sub に送信する関数
    """
    # FRED API のエンドポイント URL を設定します
    url = f"https://api.stlouisfed.org/fred/series/observations?series_id=JPNXTEXVA01CXMLM&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print("日本貿易収支データを Pub/Sub に送信しました。")
...
def main(request):
    # 最初の実行時にデータを取得
    get_japan_trade_balance_data()

    # Cloud Scheduler にジョブを作成
    job_id = "get_japan_trade_balance_job"
