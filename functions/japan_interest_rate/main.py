...
topic_id = os.environ.get('JAPAN_INTEREST_RATE_TOPIC_ID')

def get_japan_interest_rate_data():
    """
    FRED API から日本の金利データを取得し、Pub/Sub に送信する関数
    """
    # FRED API のエンドポイント URL を設定します
    url = f"https://api.stlouisfed.org/fred/series/observations?series_id=IRSTCI01JPM156N&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print("日本金利データを Pub/Sub に送信しました。")
...
def main(request):
    # 最初の実行時にデータを取得
    get_japan_interest_rate_data()

    # Cloud Scheduler にジョブを作成
    job_id = "get_japan_interest_rate_job"
    schedule = "0 0 * * 1"  # 毎週月曜日の0時0分に実行
