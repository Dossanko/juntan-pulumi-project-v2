...
topic_id = os.environ.get('TECHNICAL_INDICATORS_TOPIC_ID')

def get_technical_indicators_data(symbol):
    """
    Financial Modeling Prep API からテクニカル指標データを取得し、Pub/Sub に送信する関数
    """
    # Financial Modeling Prep API のエンドポイント URL を設定します
    url = f"https://financialmodelingprep.com/api/v3/technical_indicator/4hour/{symbol}?type=rsi,macd,bbands,stoch,adx,cci,dmi&apikey={api_key}"
    ...
    try:
        ...
        print(f"{symbol} のテクニカル指標データを Pub/Sub に送信しました。")
...
def main(request):
    # 最初の実行時にデータを取得
    for symbol in ["USDJPY", "EURUSD", "GBPJPY", "GBPUSD", "EURJPY"]:
        get_technical_indicators_data(symbol)

    # Cloud Scheduler にジョブを作成
    job_id = "get_technical_indicators_job"
