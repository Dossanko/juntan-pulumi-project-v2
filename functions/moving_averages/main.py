...
topic_id = os.environ.get('MOVING_AVERAGES_TOPIC_ID')

def get_moving_averages_data(symbol, period):
    """
    Financial Modeling Prep API から移動平均線データを取得し、Pub/Sub に送信する関数
    """
    # Financial Modeling Prep API のエンドポイント URL を設定します
    url = f"https://financialmodelingprep.com/api/v3/technical_indicator/{period}/{symbol}?type=sma,ema,wma&apikey={api_key}"
    ...
    try:
        ...
        print(f"{symbol} の移動平均線データを Pub/Sub に送信しました。")
...
def main(request):
    ...
    for symbol in ["USDJPY", "EURUSD", "GBPJPY", "GBPUSD", "EURJPY"]:
        get_moving_averages_data(symbol, "1hour")

    # Cloud Scheduler にジョブを作成
    job_id = "get_moving_averages_job"
    schedule = "0 * * * *"  # 毎時0分に実行
