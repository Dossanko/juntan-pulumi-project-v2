...
topic_id = os.environ.get('US_UNEMPLOYMENT_RATE_TOPIC_ID')

def get_us_unemployment_rate_data():
    """
    FRED API から米国失業率データを取得し、Pub/Sub に送信する関数
    """
    ...
    try:
        ...
        print("米国失業率データを Pub/Sub に送信しました。")
...
def main(request):
    # 最初の実行時にデータを取得
    get_us_unemployment_rate_data()

    # Cloud Scheduler にジョブを作成
    job_id = "get_us_unemployment_rate_job"
    schedule = "0 0 5 * *"  # 毎月5日の0時0分に実行
