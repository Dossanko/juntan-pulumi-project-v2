...
topic_id = os.environ.get('JAPAN_CURRENT_ACCOUNT_BALANCE_TOPIC_ID')

def get_japan_current_account_balance_data():
    """
    FRED API から日本の経常収支データを取得し、Pub/Sub に送信する関数
    """
    # FRED API のエンドポイント URL を設定します
    url = f"https://api.stlouisfed.org/fred/series/observations?series_id=JPNCA&api_key={api_key}&file_type=json"
    ...
    try:
        ...
        print("日本経常収支データを Pub/Sub に送信しました。")
...
def main(request):
    # 最初の実行時にデータを取得
    get_japan_current_account_balance_data()

    # Cloud Scheduler にジョブを作成
    job_id = "get_japan_current_account_balance_job"
